...
/// @file alloc_hooks.hpp

/** Allocation layer for fl::string.  Provides pluggable allocate/deallocate
    function pointers, a thread-local free-list pool for small blocks (up to
    4096 bytes across 7 size classes), and a standard-conforming pool_alloc&lt;T&gt;
    allocator adapter. */
...
/// Function pointer type for plain allocate.
using allocate_fn = void*(*)(std::size_t);
/// Function pointer type for plain deallocate.
using deallocate_fn = void(*)(void*, std::size_t);
/// Function pointer type for aligned allocate.
using allocate_aligned_fn = void*(*)(std::size_t, std::size_t);
/// Function pointer type for aligned deallocate.
...
namespace alloc_hooks {
        ...
        /// Default allocator: returns aligned memory of size @p n, or nullptr for zero.
        ...
        /// Default deallocator: frees memory previously returned by default_allocate.
    ...
    inline std::atomic<deallocate_fn>& get_deallocate_ptr() noexcept {
        ...
        /** Returns the index into POOL_CLASSES for a given size, or -1 if too large.
            ...
            Branchless bit-scan implementation: size classes are powers of two
            (64, 128, 256, ..., 4096).  For n &gt; 64, the class index is
            bit_scan_reverse((n - 1) &gt;&gt; 6) + 1
            which compiles to a single BSR/BSF or TZCNT/LZCNT instruction. */
        inline int pool_class_index(std::size_t n) noexcept {
            if (FL_UNLIKELY(n > MAX_POOL_SIZE)) return -1;
            if (FL_LIKELY(n <= 64)) return 0;
            // (n - 1) >> 6 yields a value in [1, 63] that maps cleanly to
            // the correct power-of-two class index via bit-scan-reverse.
            std::size_t const v = (n - 1) >> 6;
        #if defined(_MSC_VER)
            unsigned long idx;
            _BitScanReverse(&idx, static_cast<unsigned long>(v));
            return static_cast<int>(idx + 1);
        #else
            // __builtin_clz(0) is undefined; v >= 1 is guaranteed here.
            int const leading = __builtin_clz(static_cast<unsigned int>(v));
            return 32 - leading;  // == 31 - leading + 1
        #endif
        }

        /** Returns the usable capacity (chars, excluding null terminator) for a raw
            allocation of @p raw_size bytes routed through the pool.

            When raw_size fits a pool class, the whole class block is available —
            using this as the stored capacity avoids premature reallocation
            (e.g. constructing a 100-char string requests 101 bytes, lands in the
            128-byte class, and gets capacity 127 instead of 100, giving 27 free
            bytes before the first grow_to). */
        ...
        /** Flat TLS free-list pool.
            ...
            Layout is split into a HOT region (counts) and a COLD region (slots):
              ...
              offset  64 - 191: slots[0]     — 16 pointers for class 64   (2 lines)
              offset 192 - 319: slots[1]     — 16 pointers for class 128  (2 lines)
              offset 320 - 447: slots[2]     — 16 pointers for class 256  (2 lines)
              offset 448 - 575: slots[3]     — 16 pointers for class 512  (2 lines)
              offset 576 - 703: slots[4]     — 16 pointers for class 1024 (2 lines)
              offset 704 - 831: slots[5]     — 16 pointers for class 2048 (2 lines)
              offset 832 - 959: slots[6]     — 16 pointers for class 4096 (2 lines)

            Total: 960 bytes = 15 cache lines.  On every pool hit/miss the counts
            cache line is loaded; the relevant slots cache lines are only fetched
            when there is actual work to do there (hit -> load ptr, miss -> nothing,
            push -> store).

            POOL_SLAB_DEPTH: per-class slot capacity.  16 reduces pool eviction
            rates in allocation-heavy workloads (large string construction, rope
            flatten operations) while keeping per-thread overhead at ~1 KB. */
        constexpr int POOL_SLAB_DEPTH = 16;
        ...
        struct alignas(64) TlsFreeLists {
            ...
            /// Destructor frees all cached pool entries.  Without this, every
            /// thread that exits leaks up to ~130 KB of cached allocations.
            ~TlsFreeLists() noexcept {
                for (std::size_t cls = 0; cls < POOL_CLASSES.size(); ++cls) {
                    for (uint8_t i = 0; i < counts[cls]; ++i) {
                        void* p = slots[cls][i];
    #ifdef _WIN32
                        _aligned_free(p);
    #else
        ...
        /// Unpooled deallocate: frees memory from allocate_aligned_unpooled.
...
#if 0
        // Pool request log — compiled out by default (ENABLE_POOL_REQUEST_LOG is
        // constexpr false).  Enabled by changing the line above and providing the
        // necessary includes (<vector>, <mutex>, <fstream>).
        ...
        /// Pool-aware aligned allocate: tries the TLS free-list first, then falls
        /// back to the system allocator when the class slab is empty.
        FL_INLINE void* default_allocate_aligned(std::size_t n, std::size_t align) {
            if (FL_UNLIKELY(n == 0)) return nullptr;
            ...
            if (FL_UNLIKELY(idx < 0)) return allocate_aligned_unpooled(n, align);
        ...
        /** Pool-aware aligned deallocate: returns memory to the TLS free-list if
            the class slab has room, otherwise passes it through to the system
            allocator (recorded as an eviction for diagnostics). */
        FL_INLINE void default_deallocate_aligned(void* p, std::size_t n, std::size_t align) {
            if (FL_UNLIKELY(!p)) return;
            int idx = pool_class_index(n);
            if (FL_UNLIKELY(idx < 0)) { deallocate_aligned_unpooled(p, n, align); return; }

            TlsFreeLists& tls = get_tls_free_lists();
            if (FL_LIKELY(tls.counts[idx] < static_cast<uint8_t>(POOL_SLAB_DEPTH))) {
                ...
                // Pass the full class size (not the original request) so that
                // custom hooks using sized deallocation (mimalloc, jemalloc)
                // receive the correct allocation size.
                deallocate_aligned_unpooled(p, POOL_CLASSES[static_cast<std::size_t>(idx)], align);
                #ifndef NDEBUG
        ...
        /// Resets all pool instrumentation counters to zero.
        ...
        /// Returns an atomic snapshot of all pool instrumentation counters.
        ...
        /** Drains all cached entries in the current thread's TLS pool, returning
            the memory to the system allocator.  Useful in thread-pool environments
            where a thread processes a burst of work and then goes idle. */
        inline void drain_thread_pool() noexcept {
            TlsFreeLists& tls = get_tls_free_lists();
            for (std::size_t cls = 0; cls < POOL_CLASSES.size(); ++cls) {
                for (uint8_t i = 0; i < tls.counts[cls]; ++i) {
                    void* p = tls.slots[cls][i];
    #ifdef _WIN32
                    _aligned_free(p);
    #else
                    std::free(p);
    ...
    /** Allocates @p n bytes through either the default path or a custom hook,
        depending on whether hooks have been installed. */
    ...
    /** Deallocates memory at @p p of size @p n through either the default path
        or a custom hook. */
    ...
    /** Allocates @p n bytes with alignment @p align through either the default
        pool-aware path or a custom hook. */
    ...
    /** Deallocates aligned memory through either the default pool-aware path
        or a custom hook. */
    ...
    /** Installs custom allocation hooks.

        @param a   Plain allocate function (nullptr to keep default).
        @param d   Plain deallocate function (nullptr to keep default).
        @param aa  Aligned allocate function (nullptr to derive from @p a).
        @param da  Aligned deallocate function (nullptr to derive from @p d). */
...
inline void drain_thread_pool() noexcept { alloc_hooks::drain_thread_pool(); }
...
/// Returns the preferred allocation alignment (defined by the pool configuration).
