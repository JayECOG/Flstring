...
/// @file Debug-only thread-safety diagnostics for fl string types.
///
/// When FL_DEBUG_THREAD_SAFETY is enabled, every read, write, and move
/// operation on a string instance is tracked through an atomic state machine
/// embedded in the object.  Concurrent access violations (data races) are
/// detected at runtime and reported with a full diagnostic before aborting.
///
